...
void Matrix33CalcCommon<T>::inverse(Base& o, const Base& n)
{
    ...
    T det = (a11 * a22 * a33 - a31 * a22 * a13) + (a12 * a23 * a31 - a21 * a12 * a33) + (a13 * a21 * a32 - a11 * a32 * a23);
...
{
    const T sinV[3] = { sinf(r.x), sinf(r.y), sinf(r.z) };

    const T cosV[3] = { cosf(r.x), cosf(r.y), cosf(r.z) };
...
void Matrix34CalcCommon<T>::getRotation(Vec3& v, const Base& n)
{
    ...
    if ((1.0f - abs) < MathCalcCommon<T>::epsilon() * 10) {
        v.x = 0.0f;
        v.y = (a31 / abs) * (-numbers::pi_vd / 2);
        v.z = atan2f(-a12, -(a31 * a13));
    } else {
        v.x = atan2f(a32, a33);
        v.y = asinf(-a31);
        v.z = atan2f(a21, a11);
