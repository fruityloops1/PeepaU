    ...
    const SafeStringBase<T>& new_str)
{
    ...
    const s32 ret = replaceStringImpl_(getMutableStringTop_(), 0, getBufferSize(), target_str.cstr(),
...
{
    bool is_buffer_overflow = false;
    const s32 ret = replaceStringImpl_(getMutableStringTop_(), 0, getBufferSize(), this->cstr(),
