...
// Include shader data
#include "imgui_gx2/shaders/shader.h"
...
void ImGui_ImplGX2_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (bd->VertexBufferSize < vtx_buffer_size) {
        ...
        bd->VertexBuffer = pe::getDbgHeap()->tryAlloc(vtx_buffer_size, GX2_VERTEX_BUFFER_ALIGNMENT);
    }
    if (bd->IndexBufferSize < idx_buffer_size) {
        bd->IndexBufferSize = idx_buffer_size;
        free(bd->IndexBuffer);
        bd->IndexBuffer = pe::getDbgHeap()->tryAlloc(idx_buffer_size, GX2_INDEX_BUFFER_ALIGNMENT);
    }
...
bool ImGui_ImplGX2_CreateFontsTexture()
{
    ...
    tex->surface.dimension = GX2_SURFACE_DIM_TEXTURE_2D;
    tex->surface.use = GX2_SURFACE_USE_TEXTURE;
    tex->surface.width = width;
    tex->surface.height = height;
    tex->surface.depth = 1;
    tex->surface.mipSize = 1;
    tex->surface.format = GX2_SURFACE_FORMAT_UNORM_R8_G8_B8_A8;
    tex->surface.aa = GX2_AA_MODE1X;
    tex->surface.tile = GX2_TILE_MODE_LINEAR_ALIGNED;
    tex->viewSlicesCount = 1;
    tex->viewMipsCount = 1;
    // swapped for endianness
    tex->componentSelector = GX2_COMP_MAP(GX2_SQ_SEL_A, GX2_SQ_SEL_B, GX2_SQ_SEL_G, GX2_SQ_SEL_R);
